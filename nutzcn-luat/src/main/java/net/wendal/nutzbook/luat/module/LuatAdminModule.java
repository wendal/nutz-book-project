...
public class LuatAdminModule extends BaseModule {
    ...
    @RequiresAuthentication
    @At("/device/query")
    public NutMap query(@Param("..") Pager pager, @Param("..") LuatDevice device, @Attr("me") User me, @Attr("luat_project") LuatProject project) {
        if (project == null) {
            return _fail("select_one_project_first");
        }
    ...
    @Ok("json:full")
    public NutMap uploadPkg(@Param("file") TempFile tf, @Param("content") String content, @Attr("luat_project") LuatProject project, @Attr("me") User me) throws IOException {
        if (project == null)
        ...
        pkg.setFirmwareName(tmp[2].substring(0, tmp[2].indexOf('.')));
    ...
    public NutMap pkgQuery(@Param("..") Pager pager, @Param("..") LuatUpgradePackage pkg, @Attr("me") User me, @Attr("luat_project") LuatProject project) {
        if (project == null)
            return _fail("select_on_project_first");
        Cnd cnd = Cnd.where("userId", "=", me.getId()).and("projectId", "=", project.getId());
    ...
    public NutMap upgradePlanUpdate(@Param("..") LuatUpgradePlan device, @Attr("me") User me) {
        ...
        device.setUpdateTime(new Date());
        dao.update(device, "pkgId|imeis|iccids|versionEq|versionNEq|provinces|cities|content|updateTime");
        return _ok(device);
    }
    ...
    public NutMap upgradePlanEnable(@Param("id") long id, @Param("enable") boolean enable, @Attr("me") User me) {
    ...
    public NutMap query(@Param("..") Pager pager, @Param("..") LuatUpgradePlan plan, @Attr("me") User me, @Attr("luat_project") LuatProject project) {
        if (project == null)
            return _fail("select_on_project_first");
        Cnd cnd = Cnd.where("userId", "=", me.getId()).and("projectId", "=", project.getId());
        _like(cnd, "name", plan.getName());
